...
ensure_bun() {
  if ! have bun; then sudo -u "$SERVICE_USER" -H bash -lc 'curl -fsSL https://bun.sh/install | bash'; fi
  local USER_BUN="${SERVICE_HOME}/.bun/bin/bun"
  if [[ -x "$USER_BUN" ]]; then sudo install -m 0755 "$USER_BUN" /usr/local/bin/bun; fi
...
# Private HOME + XDG in repo (avoid real \$HOME writes)
